export default {
    Det2x2(data) {
        return (data[0][0] * data[1][1] - data[0][1] * data[1][0]).toFixed(2);
    },
    Pasos2x2(data) {
        let Pasos = [];
        Pasos[0] =
        ...
        Pasos[1] =
            "(" +
            (data[0][0] * data[1][1]).toString() +
            ")-" +
            "(" +
            (data[0][1] * data[1][0]).toString() +
            ")";
    ...
    PasosSistEcu2(data) {
        ...
        if (Dsist == 0) {
            Pasos[3] = "La solucion es incompatible(No tiene solución)";
            return Pasos;
        }
        Pasos[3] =
    ...
    dividirMatrizPorEscalar(escalar, matriz) {
        ...
        for (var i = 0; i < matriz.length; i++) {
            ...
            for (var j = 0; j < matriz[i].length; j++) {
                fila.push((matriz[i][j] / escalar).toFixed(2));
            }
    ...
    PasosInverza(data) {
        ...
        const adjunta = this.calcularAdjunta(traspuesta);
        Pasos[0] =
            "calcular el determinante de la Matriz y determinar si es ≠ 0";
        switch (data.length) {
            case 2:
                Dt = this.Det2x2(data);
            ...
            case 3:
                Dt = this.Det3x3(data);
                Pasos[1] = this.Pasos3x3(data);
                break;
            case 4:
                Dt = this.Det4x4(data);
                Pasos[1] = this.Pasos4x4(data);
        }
        ...
        console.log(Inversa);
        Pasos[2] =
            "determinar la traspuesta combiando las filas por columnas o viceverza ";

        Pasos[3] = traspuesta;
        Pasos[4] =
            "determinar la matriz adjunta de la traspuesta.(metodo de cofactores)";

        Pasos[5] = adjunta;
        Pasos[6] = "dividir la matriz adjunta por el determinante (cada valor de la matriz se divide por el determinante)";
